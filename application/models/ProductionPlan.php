...
class ProductionPlan extends CI_Model
{
    ...
    */
    ...
    {
        $query = $this->db->query('SELECT plan_by_hours.*, interruptions.interruption_name, not_planned_interruptions.interruption_name as not_planned_interruption_name from plan_by_hours LEFT JOIN interruptions ON plan_by_hours.interruption_id = interruptions.interruption_id LEFT JOIN not_planned_interruptions ON plan_by_hours.not_planned_interruption_id = not_planned_interruptions.interruption_id WHERE plan_id = ' . $this->plan_id . " ORDER BY time ASC");
